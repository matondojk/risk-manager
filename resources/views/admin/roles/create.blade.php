            ...
            <div class="bg-white dark:bg-gray-900 shadow-sm sm:rounded-lg border border-gray-100 dark:border-gray-800 overflow-hidden">
                <form action="{{ route('roles.store') }}" method="POST" class="p-6 space-y-6">
                        ...
                        <div class="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-4 border border-gray-200 dark:border-gray-700 p-4 rounded-md">
                            @foreach($permissions as $permission)
                                <label class="inline-flex items-center">
                                    <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" class="rounded border-gray-300 dark:border-gray-700 text-gray-900 shadow-sm focus:ring-gray-900 dark:focus:ring-gray-300" {{ in_array($permission->name, old('permissions', [])) ? 'checked' : '' }}>
                    ...
                    <div class="flex items-center justify-end gap-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                        ...
                        <x-primary-button>Criar Perfil</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
